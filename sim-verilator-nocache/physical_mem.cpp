...
void PhysicalMemory::record_missing_read(PmemMissingReadKind kind) const {
    switch (kind) {
    case PmemMissingReadKind::ColdPage: ++m_missing_read_stats.cold_page; break;
    case PmemMissingReadKind::PdsColdPage: ++m_missing_read_stats.pds_cold_page; break;
    case PmemMissingReadKind::AllocationPadding:
        ++m_missing_read_stats.allocation_padding;
        break;
    case PmemMissingReadKind::OutOfBounds: ++m_missing_read_stats.out_of_bounds; break;
    }
}
...
bool PhysicalMemory::read(paddr_t paddr, void* data_, uint64_t size) const {
    ...
    if (m_map.find(first_page_base) == m_map.end()) {
        ...
        case PmemMissingReadKind::ColdPage:
            logger->warn(
                "PMEM cold page read at 0x{:x} ({} bytes) inside buffer allocation; returning zeros",
                paddr, size);
            break;
        case PmemMissingReadKind::PdsColdPage:
            logger->warn(
                "PMEM PDS cold page read at 0x{:x} ({} bytes); returning zeros", paddr, size);
            break;
        case PmemMissingReadKind::AllocationPadding:
            logger->warn(
                "PMEM allocation-padding read at 0x{:x} ({} bytes) inside aligned buffer extent; returning zeros",
                paddr, size);
            break;
        case PmemMissingReadKind::OutOfBounds:
            logger->error(
                "PMEM out-of-bounds read at 0x{:x} ({} bytes); returning zeros", paddr, size);
            break;
        }
