...
class SimpleModuleSimple extends Module {
  ...
  // 初始cnt_mask为0
  val cnt_mask = VecInit(Seq.fill(numgroup+1)(0.U(log2Ceil(numgroup).W)))

  // 遍历current_mask，对于每个为true的位，记录其索引
  for (x <- 0 until numgroup) {
    when(io.current_mask(x)) {
      printf("current_mask(%d)\n", x.U)
      io.current_mask_index(cnt_mask(x)) := x.U
      for(i <- x until numgroup){
        cnt_mask(i+1) := cnt_mask(x) + 1.U
      }
    }
  }
}
...
object SimpleModuleVerilog extends App {
  ...
  (new chisel3.stage.ChiselStage).emitVerilog(new SimpleModuleSimple, Array("--target-dir", targetDir) ++ args)
}
