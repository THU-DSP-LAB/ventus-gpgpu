...
import FPUv2.TCCtrl
import TensorCore.{TC_MMA888,TC_MMAInput,TC_MMAOutput}//, TC_MMA1688Input, TC_MMA1688Output}
import chisel3._
    ...
    def apply() = {
      ...
      new TC_MMAInput(new TCCtrl(32, 1)).Lit(
        _.data_in -> new vExeData().Lit(
          _.in1 -> Vec(num_thread, UInt(xLen.W)).Lit(
            (0 until num_thread).map{i => i-> RA(i).U}:_*
          ),
          _.in2 -> Vec(num_thread, UInt(xLen.W)).Lit(
            (0 until num_thread).map{i => i-> RB(i).U}:_*
          ),
          _.in3 -> Vec(num_thread, UInt(xLen.W)).Lit(
            (0 until num_thread).map{i => i-> RC(i).U}:_*
          ),
          _.mask -> Vec(num_thread,Bool()).Lit(
            (0 until num_thread).map{ i => i -> false.B}:_*
          ),
