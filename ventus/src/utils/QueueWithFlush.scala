//This file comes from chisel 3.5
package utils
...
class QueueWithDataFlushIO[T <: Data](
  private val gen: T,
  val entries:     Int,
...
/** A hardware module implementing a Queue
  ...
  */
@chiselName
class QueueWithDataFlush[T <: Data](
  val gen:            T,
  val entries:        Int,
  ...
  val io = IO(new QueueWithDataFlushIO(genType, entries, hasFlush))
  ...
  when(flushdata) {
    ram.foreach{_:=0.U.asTypeOf(genType)}
  }
...
object QueueWithDataFlush {
  ...
  /** Create a [[Queue]] and supply a [[DecoupledIO]] containing the product.
    ...
    *             The `valid` signals are coupled.
    * @param useSyncReadMem True uses SyncReadMem instead of Mem as an internal memory element.
    * @param flush Optional [[Bool]] signal, if defined, the [[QueueWithDataFlush.hasFlush]] will be true, and connect correspond
  ...
  def apply[T <: Data](
    ...
    if (entries == 0) {
      ...
      val q = Module(new QueueWithDataFlush(chiselTypeOf(enq.bits), entries, pipe, flow, useSyncReadMem, flush.isDefined))
...
class QueueWithFlush[T <: Data](
  ...
  val io = IO(new QueueWithFlushIO(genType, entries, hasFlush))
...
object QueueWithFlush {
   ...
   *             The `valid` signals are coupled.
   * @param useSyncReadMem True uses SyncReadMem instead of Mem as an internal memory element.
   * @param flush Optional [[Bool]] signal, if defined, the [[QueueWithFlush.hasFlush]] will be true, and connect correspond
   *              signal to [[Queue]] instance.
   * @return output (dequeue) interface from the queue.
   *
   * @example {{{
  ...
  def apply[T <: Data](
    ...
    if (entries == 0) {
      ...
      val q = Module(new QueueWithFlush(chiselTypeOf(enq.bits), entries, pipe, flow, useSyncReadMem, flush.isDefined))
      q.io.flush.zip(flush).foreach(f => f._1 := f._2)
      q.io.enq.valid := enq.valid // not using <> so that override is allowed
      q.io.enq.bits := enq.bits
      enq.ready := q.io.enq.ready
