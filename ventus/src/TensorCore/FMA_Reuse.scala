...
//  int4GetResult.io.in := RegEnable(int4Extender.io.out1, io.regEnables(0)) * RegEnable(int4Extender.io.out2, io.regEnables(0))
  ...
  io.result_INT4_out_bc := io.result(7,0)//int4GetResult.io.out_bc
  io.result_INT4_out_ac := io.result(15,8) //int4GetResult.io.out_ac
  io.sum := 0.U
  io.carry := 0.U
}
...
class Int4Extender_12 extends Module {
  ...
  io.out1 := Cat(io.inA,0.U(4.W), io.inB)
  // 将io.inC的位放在low位，然后在high位附加一个12位的零值
  io.out2 := Cat(0.U(8.W),io.inC)
}
