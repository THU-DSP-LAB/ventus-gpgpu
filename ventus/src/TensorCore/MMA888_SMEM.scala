...
class TensorCore_MixedPrecision(DimM: Int, DimN: Int, DimK: Int, xDatalen:Int = 16, tcctrl:TCCtrl) extends Module {
  //  mnk defined as cuda.
  //  m8n8k8
  //  xDatalen: data bit len === xLen. FP16: xDatalen=16
  //  Compute: C[m8n8] = A[m8k8] * B[k8n8] + C[m8n8]
  ...
  // Init TC Computation Array
  val TCComputation = Module(new TC_ComputationArray_MixedPrecision(16,8,4,8,tcCtrl = tcctrl))
  dontTouch(TCComputation.io.out)
...
class TC_MMA888_SMEM(DimM: Int, DimN: Int, DimK: Int, xDatalen:Int = 16, tcCtrl: TCCtrl) extends Module {
  //  mnk defined as cuda.
  //  m8n8k8
  //  xDatalen: data bit len === xLen. FP16: xDatalen=16
  //  Compute: C[m8n8] = A[m8k8] * B[k8n8] + C[m8n8]
  ...
  switch(stateReg) {
    is(sIdle) {
      when(io.in.fire) {
        ...
        Load4SMEM.io.in.bits.mask := Vec(num_thread, true.B) //TODO. all true temp
