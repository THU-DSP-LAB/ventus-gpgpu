...
object SingleSMNoCacheGen {
  private def autoDirName(
    ...
    val sharedmemBandwidthBits = HardwareConfig.derivedSharedmemBandwidthBits(
  ...
  def elaborate(
    ...
    @arg(name = "dcache-nsets", doc = "address decomposition set count used by LSU") dcacheNSets: Int = HardwareConfig.defaults.dcacheNSets,
    @arg(name = "dcache-nways", doc = "address decomposition way count") dcacheNWays: Int = HardwareConfig.defaults.dcacheNWays,
    @arg(name = "dcache-block-words", doc = "cacheline words used by LSU/sharedmem interfaces") dcacheBlockWords: Int = HardwareConfig.defaults.dcacheBlockWords,
    @arg(name = "dcache-mshr-entry", doc = "MSHR entries used by LSU metadata") dcacheMshrEntry: Int = HardwareConfig.defaults.dcacheMshrEntry,
    @arg(name = "dcache-mshr-sub-entry", doc = "sub entries per MSHR") dcacheMshrSubEntry: Int = HardwareConfig.defaults.dcacheMshrSubEntry,
    @arg(name = "dcache-wshr-entry", doc = "writeback queue entries") dcacheWshrEntry: Int = HardwareConfig.defaults.dcacheWshrEntry,
    ...
    (new ChiselStage).execute(
      ...
      Seq(ChiselGeneratorAnnotation(() => new GPGPU_top_nocache))
    )
