  ...
  if(SPIKE_OUTPUT) {
    ...
    when(io.out_LSU.fire&&io.out_LSU.bits.ctrl.mem/*&&io.out_LSU.bits.ctrl.wid===wid_to_check.U*/){
      ...
      when(!io.out_LSU.bits.ctrl.isvec){
        printf(p"x${io.out_LSU.bits.ctrl.reg_idxw} op ${io.out_LSU.bits.ctrl.mop} base ${Hexadecimal(io.out_LSU.bits.in1(0))} bias ${Hexadecimal(io.out_LSU.bits.in2(0))}\n")
      }.otherwise{
        printf(p"v${io.out_LSU.bits.ctrl.reg_idx3} op ${io.out_LSU.bits.ctrl.mop} base ${Hexadecimal(io.out_LSU.bits.in1(0))} bias ")
        io.out_LSU.bits.in2.reverse.foreach(x => printf(p"${Hexadecimal(x.asUInt)} "))
        printf(p"\n")
      }
    }
