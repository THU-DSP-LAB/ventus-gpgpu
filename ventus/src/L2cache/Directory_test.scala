...
//  }
  ...
  io.result.bits.hit := Mux(RegNext(flush_issue), true.B, (hit || (setQuash && tagMatch )||(setQuash_1 &&tagMatch_1)) && (!about_replace))
  io.result.bits.way  := Mux(RegNext(flush_issue), RegNext(flush_way),Mux(hit, OHToUInt(hits), Mux(setQuash && tagMatch,RegNext(io.write.bits.way),Mux(setQuash_1&&tagMatch_1,io.write.bits.way,victimWay))))
