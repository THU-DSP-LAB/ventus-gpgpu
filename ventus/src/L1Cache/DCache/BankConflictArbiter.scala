...
class DataCrossbar(implicit p: Parameters) extends DCacheModule {
  val io=IO(new Bundle {
...
class AddrBundle1T(implicit p: Parameters) extends DCacheBundle {
...
class coreReqArb1T (implicit p:Parameters) extends DCacheBundle{
  val activeMask = Bool()
  val bankIdx = UInt(BankIdxBits.W)
  val AddrBundle = new AddrBundle1T
}
class coreReqArb(implicit p:Parameters) extends DCacheBundle{
  val isWrite = Bool()
  val enable = Bool()
  val perLaneAddr = Vec(NLanes,new DCachePerLaneAddr)//OT = One Thread
}

class BankConflictArbiter(implicit p: Parameters) extends DCacheModule {
  val io = IO(new Bundle{
