...
class DataCachev2(SV: Option[mmu.SVParam] = None)(implicit p: Parameters) extends DCacheModule{
  ...
  // Reserve the final slot for every resident st1 request, not only requests
  // currently classified as RTAB-bound. Fill/MSHR state can change that
  // classification on the next cycle; admitting an st0 RTAB hit in the prior
  // cycle would otherwise fill the table and deadlock replay behind st1.
