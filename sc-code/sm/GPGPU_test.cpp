...
int sc_main(int argc, char *argv[])
{
    ...
    cta_impl.CTA_INIT();
    BASE_impl->mtd = cta_impl.mtd;
    BASE_impl->metafile = "../../testcase/" + metafile;
    BASE_impl->datafile = "../../testcase/" + datafile;

    for (auto &warp_ : BASE_impl->WARPS)
        if (warp_.is_warp_activated)
            BASE_impl->ev_issue_list &= warp_.ev_issue;
    ...
    for (int i = 0; i < num_warp; i++)
    {
        ...
        for (int j = 0; j < 32; j++)
            sc_trace(tf[i], BASE_impl->testCSR[j], "test_CSR.data(" + std::to_string(j) + ")");
        ...
        sc_trace(tf[i], BASE_impl->WARPS[i].fetch_valid2, "fetch_valid2");
        sc_trace(tf[i], BASE_impl->WARPS[i].pc, "pc");
        sc_trace(tf[i], BASE_impl->WARPS[i].fetch_ins, "fetch_ins");
        sc_trace(tf[i], BASE_impl->WARPS[i].decode_ins, "decode_ins");
